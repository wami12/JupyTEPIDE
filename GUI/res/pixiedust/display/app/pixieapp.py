...
def runPixieApp(app, parent_pixieapp=None, entity=None, **kwargs):
    ...
    if isinstance(app, PixieDustApp):
        ...
        if parent_pixieapp is not None:
            instance_app.parent_pixieapp = parent_pixieapp if isinstance(parent_pixieapp, PixieDustApp) else \
...
class PixieDustApp(Display):
    ...
    def getOptionValue(self, optionName):
        ...
        if option is None:
            ...
            option = getattr(self.entity, optionName) if self.entity is not None and hasattr(self.entity,
                                                                                             optionName) else None
            ...
            try:
                ...
                if isinstance(retValue, string_types):
                    ...
                    if dialogRoot is not None:
                        jsOnLoad = """pixiedust.dialogRoot="{}";\n{}""".format(self.renderTemplateString(dialogRoot),
                                                                               jsOnLoad)
                    if body is not None:
...
def PixieApp(cls):
    ...
    PixieDustApp.routesByClass[clsName]["routes"] = [p[a[1]] for a in
                                                     sorted([(len(a[0]), i) for i, a in enumerate(p)], reverse=True)]
    ...
    def run(self, entity=None, **kwargs):
        ...
        display_call = "display" if ShellAccess['display'] is not None and ShellAccess[
            'display'].__module__ == "pixiedust.display" else "pixiedust.display"
        s = "{}({}{})".format(display_call, var, reduce(lambda k, v: k + "," + v[0] + "='" + str(v[1]) + "'", opts, ""))
