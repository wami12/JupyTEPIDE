...
define([
    ...
    //Jupyteo.leafletMap initialization
    var load_map = function(map_container) {
        mymap = L.map(map_container,{drawControl:true});
        Jupyteo.leafletMap = mymap;
        toggle_map_action('mapClick');
        Jupyteo.marker = new L.Marker();
        Jupyteo.leafletMap.tmpShapeID = -1;
        Jupyteo.leafletMap.tmpShapeVertexArray = [];
        Jupyteo.leafletMap.tmpShapeWKT = 'undefined';

        Jupyteo.leafletMap.setView([0,0], 1).on('click', onMapClick);
        L.control.scale().addTo(Jupyteo.leafletMap);

        //this enables all tools from leaflet.pm plugin - can be used in near future
        //todo:leaflet pm controls adding
        //Jupyteo.leafletMap.pm.addControls();
        ...
        //Remember tmpShapeID
        Jupyteo.leafletMap.on('pm:create', function(e) {
            ...
            Jupyteo.leafletMap.tmpShapeID = tmpShapeID;

        });

        //remember each vertex in tmpShapeVertexArray
        Jupyteo.leafletMap.on('pm:drawstart', function(e) {
            var layer = e.workingLayer;
            Jupyteo.leafletMap.tmpShapeVertexArray = [];
            if(Jupyteo.marker){Jupyteo.marker.remove()};
            remove_tmp_shape();
            ...
            layer.on('pm:vertexadded', function(e) {
                //console.log(e.latlng);
                if (e.shape=='Poly') {
                    Jupyteo.leafletMap.tmpShapeVertexArray.push(e.latlng);
                }
            });
        ...
        Jupyteo.leafletMap.on('pm:drawend',function(e){
            if (e.shape=='Poly'|e.shape=='Rectangle'){
                toggle_map_action('mapClick');

                if (e.shape=='Rectangle' & typeof Jupyteo.leafletMap.tmpShapeVertexArray != 'undefined'
                    & Jupyteo.leafletMap.tmpShapeVertexArray.length>0 ){
                    //must build rectangle for WKT from two collected points
                    var tmpVtxArr = Jupyteo.leafletMap.tmpShapeVertexArray;
                    ...
                    Jupyteo.leafletMap.tmpShapeVertexArray=tmpVtxArr;

                }

                var WKTstr = 'POLYGON((';
                var vertexStr='';
                var numVertex = Jupyteo.leafletMap.tmpShapeVertexArray.length;
                 for(var i=0;i<numVertex;i++){
                     vertexStr = Jupyteo.leafletMap.tmpShapeVertexArray[i].lng +' '+ Jupyteo.leafletMap.tmpShapeVertexArray[i].lat;

                     if(i==numVertex-1){
                         WKTstr=WKTstr+ vertexStr+','+Jupyteo.leafletMap.tmpShapeVertexArray[0].lng +' '+ Jupyteo.leafletMap.tmpShapeVertexArray[0].lat+'))';
                     }
                     else WKTstr=WKTstr+ vertexStr+',';
                 }
             }
             Jupyteo.leafletMap.tmpShapeWKT = WKTstr;
    ...
    var load_tileLayer = function(url_,atrib) {
        return L.tileLayer(url_, atrib).addTo(Jupyteo.leafletMap);
    };
    ...
    var load_wmsLayer = function (url_,atrib){
        return L.tileLayer.wms(url_,atrib).addTo(Jupyteo.leafletMap);
    ...
    //*** load_mapboxLayer ***
    //initial map loaded into Jupyteo UI
    ...
    var load_initialBaseLayers = function() {
        Jupyteo.leafletMap.layers = {};

        Jupyteo.leafletMap.layers.mapbox = load_tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoibWFwYm94IiwiYSI6ImNpejY4NXVycTA2emYycXBndHRqcmZ3N3gifQ.rJcFIG214AriISLbB6B5aw', {
            maxZoom: 18,
            attribution: 'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, ' +
            '<a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, ' +
            'Imagery © <a href="http://mapbox.com">Mapbox</a>',
        ...
        Jupyteo.leafletMap.layers.osm = load_tileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            maxZoom: 20,
        ...
        //Base layers ****
        var baseLayers = {
            "Mapbox:streets":Jupyteo.leafletMap.layers.mapbox,
            "OSM":Jupyteo.leafletMap.layers.osm
        };

        var overlays ={};
        Jupyteo.leafletMap.control = add_layerControls(baseLayers,overlays);
        set_view([52,21],3);
    };

    //*** set_view ***
    var set_view = function(center,zoom){
        Jupyteo.leafletMap.setView(center, zoom);
    };
    ...
    var add_marker = function(center,popup_) {
        ...
        L.marker(center, parameters).addTo(Jupyteo.leafletMap)
    ...
    function onMapClick(e) {
        ...
        if (Jupyteo.mapClick){
            ...
            //     .openOn(Jupyteo.leafletMap);
        }

        //point marker for RESTO searching - "click" mode disabled
        else if (Jupyteo.mapAddPoint){
            Jupyteo.marker = new L.Marker(e.latlng,{draggable:true,icon:markerIcon});
            Jupyteo.leafletMap.addLayer(Jupyteo.marker);
            Jupyteo.leafletMap.tmpShapeWKT='MULTIPOINT(('+Jupyteo.marker._latlng.lng+' '+Jupyteo.marker._latlng.lat+'))';
            toggle_map_action('mapClick');
            $('#insertSearchShapeButton').removeClass('selected');
        }

        //polygon shape for RESTO searching - "click" mode disabled - look at load_map()
        else if (Jupyteo.mapAddPoly){
        ...
        else if (Jupyteo.mapAddRectangle){
    ...
    var toggle_map_action = function(action){
        Jupyteo.mapAddPoint=false;
        Jupyteo.mapAddRectangle = false;
        Jupyteo.mapAddPoly = false;
        Jupyteo.mapClick=false;

        if (action=='mapAddPoint'){Jupyteo.mapAddPoint=true}
        else if (action=='mapAddRectangle'){Jupyteo.mapAddRectangle=true}
        else if (action=='mapAddPoly'){Jupyteo.mapAddPoly=true}
        else if (action=='mapClick'){Jupyteo.mapClick=true}
    };

    //*** remove_tmp_shape ***
    //removes from map, temporary shape drawn with leaflet pm extension (for RESTO searching)
    var remove_tmp_shape = function(){
        if(Jupyteo.leafletMap.tmpShapeWKT){
            if (Jupyteo.leafletMap._layers[Jupyteo.leafletMap.tmpShapeID]) {
                Jupyteo.leafletMap._layers[Jupyteo.leafletMap.tmpShapeID].remove()
            };
            //Jupyteo.leafletMap.pm.disableDraw();
            Jupyteo.leafletMap.tmpShapeID = -1;
            Jupyteo.leafletMap.tmpShapeVertexArray = [];
            Jupyteo.leafletMap.tmpShapeWKT = 'undefined';
        }
    ...
    var draw_rect_tmp_marker = function(options){
        Jupyteo.leafletMap.pm.enableDraw('Rectangle',options);
    };
    //*** draw_poly_tmp_marker ***
    var draw_poly_tmp_marker = function(options){
        Jupyteo.leafletMap.pm.enableDraw('Poly',options);
    };
    ...
    var add_circle = function(center,radius,popup_,parameters_){
        return L.circle(center, radius, parameters_).addTo(Jupyteo.leafletMap).bindPopup(popup_);
    };
    ...
    var add_polygon = function(points,popup_,parameters_){
        return L.polygon(points,parameters_).addTo(Jupyteo.leafletMap).bindPopup(popup_);
    };
    ...
    var add_polyline = function(latlngs,options,popup_){
        return L.polyline(latlngs,options).addTo(Jupyteo.leafletMap).bindPopup(popup_);
    };

    //*** add_layerControls ***
    var add_layerControls = function(baseLayers,overlays){
        return L.control.layers(baseLayers,overlays,{collapsed:true}).addTo(Jupyteo.leafletMap);
    ...
    var load_geojson = function(){
        ...
        L.geoJSON(geojsonFeature).addTo(Jupyteo.leafletMap);
    ...
    var load_test_polygon = function(popupText){
        L.polygon([
            ...
            [51.50165, -0.2029],
            [51.49577, -0.15003]
        ]).addTo(Jupyteo.leafletMap).bindPopup(popupText);
